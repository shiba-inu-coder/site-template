<template>
  ...
  >
    ...
    >
      <b class="block truncate text-step-8 text-ui-heading">{{ bonus }}</b>
      <span
        v-if="terms"
        class="block truncate text-ui-muted"
        >{{ terms }}</span
      >
    </div>
    <CtaButtonLayout
...
const { bonus, terms, buttonLabel, buttonLink, hasCta } = usePageOffer();
</script>
