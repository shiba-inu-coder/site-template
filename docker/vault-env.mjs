// Превращение записи Vault в переменные окружения Nitro.
//
// Модуль чистый и лежит отдельно от точки входа: ошибка в этом отображении
// означает сайт, поднявшийся с пустым конфигом, — молча и без единой строки
// в логе. Поэтому же здесь живёт проверка обязательных ключей.
...
/**
 * Ключ runtimeConfig → имя переменной окружения. Префикс обязателен:
 * nuxt.config.ts выполняется только на сборке, а в контейнере значения
 * подставляет Nitro, и он читает окружение со своим префиксом.
 */
...
/**
 * Переносит значения в окружение процесса, не затирая уже заданные: то, что
 * пришло из стека, должно побеждать содержимое Vault. Это единственный способ
 * подменить один ключ в аварии, не трогая хранилище.
 */
